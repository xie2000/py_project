...
class Spider:
    ...
    def saveIcon(self,iconURL,name):
        print 'saveicon:'+iconURL
        splitPath = iconURL.split('.')
        fTail = splitPath.pop()
        imageNname = splitPath.pop();
        fileName = name + '/'+imageNname+'.' + fTail
        self.saveImg(iconURL,fileName)

    #保存个人简介
    def saveBrief(self,content,name):
        print 'saveBrief:'+content
    ...
    def savePageInfo(self,pageIndex):
        ...
        for item in contents:
            ...
            self.mkdir(dir+item[2])
            #保存个人简介
            #self.saveBrief(brief,item[2])
            #保存头像
            self.saveIcon(item[1],dir+item[2])
            #保存图片
